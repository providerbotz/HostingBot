#POWERED BY @PROVIDERBOTZ & @LazyProvider
#DONT SHARE/SELL THIS WITHOUT PERMISSION !
#OTHERWISE @LAZYPROVIDER DONT WANT TO BECAME YOUR SECOND FATHER !!
# -*- coding: utf-8 -*-
import telebot
import subprocess
import os
...
@app.route('/')
def home():
    return "🤖 Provider Hosting Bot 🦁 is Running!"
@app.route('/health')
def health():
...
OWNER_ID = 7931847651
ADMIN_ID = 7484783061
YOUR_USERNAME = '@LazyProvider'
UPDATE_CHANNEL = 'https://t.me/ProviderBotz'
# FOLDER SETUP / NO NEED TO CHANGE
...
║ 👤 𝐔𝐬𝐞𝐫: {script_owner_id}
║ 🔄 𝐀𝐭𝐭𝐞𝐦𝐩𝐭: {attempt}/{max_attempts}
╚══════════════════════════════╝
"""
...
║     ✅ <b>𝐒𝐂𝐑𝐈𝐏𝐓 𝐑𝐔𝐍𝐍𝐈𝐍𝐆</b> ✅               ║
╠════════════════════════════╣
║ 📄 <b>𝐅𝐢𝐥𝐞:</b> <code>{file_name[:25]}</code>
║ 🆔 <b>𝐏𝐈𝐃:</b> {process.pid}
║ ⏱️ <b>𝐒𝐭𝐚𝐫𝐭𝐞𝐝:</b> {datetime.now().strftime('%H:%M:%S')}
╚══════════════════════════════════════╝
"""
            try:
...
╠════════════════════════════╣
║ 📄 <b>𝐅𝐢𝐥𝐞:</b> <code>{file_name[:25]}</code>
║ ❗ <b>𝐄𝐱𝐢𝐭 𝐂𝐨𝐝𝐞:</b> {process.returncode}
╠════════════════════════════╣
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>𝐏𝐑𝐎𝐕𝐈𝐃𝐄𝐑𝐁𝐎𝐓𝐙: 𝐍𝐎𝐃𝐄.𝐉𝐒 𝐅𝐀𝐈𝐋𝐄𝐃</b> ❌    ║
╠══════════════════════════════════════╣
║ 📄 <b>𝐅𝐢𝐥𝐞:</b> <code>{file_name[:25]}</code>
║ ❗ <b>𝐄𝐱𝐢𝐭 𝐂𝐨𝐝𝐞:</b> {process.returncode}
╠══════════════════════════════════════╣
...
def get_file_actions_keyboard(file_name, is_running=False):
    ...
    if is_running:
        markup.add(
            types.InlineKeyboardButton("🛑 𝐒𝐓𝐎𝐏", callback_data=f"stop_{file_name}"),
            types.InlineKeyboardButton("📋 𝐋𝐎𝐆𝐒", callback_data=f"logs_{file_name}")
        )
        markup.add(
            types.InlineKeyboardButton("🔄 𝐑𝐄𝐒𝐓𝐀𝐑𝐓", callback_data=f"restart_{file_name}")
        )
    else:
        markup.add(
            types.InlineKeyboardButton("▶️ 𝐑𝐔𝐍", callback_data=f"run_{file_name}"),
            types.InlineKeyboardButton("🗑️ 𝐃𝐄𝐋𝐄𝐓𝐄", callback_data=f"delete_{file_name}")
        )
        markup.add(
            types.InlineKeyboardButton("📥 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃", callback_data=f"download_{file_name}"),
            types.InlineKeyboardButton("📝 𝐄𝐃𝐈𝐓", callback_data=f"edit_{file_name}")
        )
        markup.add(types.InlineKeyboardButton("🔙 𝐁𝐀𝐂𝐊", callback_data="back_to_files"))
    return markup
...
def start_command(message):
    ...
    if bot_locked and user_id not in admin_ids and user_id != OWNER_ID:
        ...
        return
    welcome_text = f"""
👋 ᴡᴇʟᴄᴏᴍᴇ, <b>{message.from_user.first_name}</b>!

> 📤 ᴜᴘʟᴏᴀᴅ & ʜᴏꜱᴛ ʏᴏᴜʀ ʙᴏᴛ ꜰɪʟᴇꜱ  
> 🚀 ʀᴜɴ ᴘʏᴛʜᴏɴ & ɴᴏᴅᴇ.ᴊꜱ ꜱᴄʀɪᴘᴛꜱ  
> 📊 ᴍᴏɴɪᴛᴏʀ ʏᴏᴜʀ ʀᴜɴɴɪɴɢ ʙᴏᴛꜱ  
> 💾 ᴍᴀɴᴀɢᴇ ʏᴏᴜʀ ꜰɪʟᴇꜱ ᴇᴀꜱɪʟʏ  

📌 <b>ʏᴏᴜʀ ʟɪᴍɪᴛꜱ:</b>  
📁 ꜰɪʟᴇꜱ: {get_user_file_count(user_id)}/{int(get_user_file_limit(user_id)) if get_user_file_limit(user_id) != float('inf') else '∞'}  
💳 ꜱᴛᴀᴛᴜꜱ: {'👑 ᴏᴡɴᴇʀ' if user_id == OWNER_ID else '⭐ ᴀᴅᴍɪɴ' if user_id in admin_ids else '🌟 ᴘʀᴇᴍɪᴜᴍ' if user_id in user_subscriptions else '👤 ꜰʀᴇᴇ'}  

ᴜꜱᴇ ᴛʜᴇ ʙᴜᴛᴛᴏɴꜱ ʙᴇʟᴏᴡ ᴛᴏ ɴᴀᴠɪɢᴀᴛᴇ! ⬇️
"""
...
║ • /delete - ᴅᴇʟᴇᴛᴇ ᴀ ꜰɪʟᴇ      ║ 
║                                               ║
║ <b>🤖 ʙᴏᴛ ᴄᴏɴᴛʀᴏʟ:</b>            ║
║ • /run - ʀᴜɴ ᴀ ꜱᴄʀɪᴘᴛ
║ • /stop - ꜱᴛᴏᴘ ᴀ ʀᴜɴɴɪɴɢ ꜱᴄʀɪᴘᴛ
║ • /logs - ᴠɪᴇᴡ ꜱᴄʀɪᴘᴛ ʟᴏɢꜱ
║ • /running - ꜱᴇᴇ ʀᴜɴɴɪɴɢ ꜱᴄʀɪᴘᴛꜱ
║
║ <b>📊 ɪɴꜰᴏʀᴍᴀᴛɪᴏɴ:</b>
║ • /stats - ʙᴏᴛ ꜱᴛᴀᴛɪꜱᴛɪᴄꜱ
║ • /speed - ᴄʜᴇᴄᴋ ꜱᴘᴇᴇᴅ
║ • /status - ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ꜱᴛᴀᴛᴜꜱ
║
║ <b>🔧 ᴏᴛʜᴇʀ:</b>
║ • /start - ʀᴇꜱᴛᴀʀᴛ ʙᴏᴛ
║ • /help - ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ
...
def stats_command(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐆𝐚𝐭𝐡𝐞𝐫𝐢𝐧𝐠 𝐬𝐭𝐚𝐭𝐬...", duration=2)
...
def speed_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 𝐬𝐩𝐞𝐞𝐝...", duration=2)
    ...
    speed_text = f"""
╔══════════════════════════════════════╗
║        ⚡ <b>𝐇𝐎𝐒𝐓𝐈𝐍𝐆 𝐁𝐎𝐓 𝐒𝐏𝐄𝐄𝐃</b> ⚡        ║
╠══════════════════════════════════════╣
║
║  🏓 <b>𝐋𝐀𝐓𝐄𝐍𝐂𝐘:</b> {latency:.2f}𝐦𝐬
║  🖥️ <b>𝐂𝐏𝐔:</b> {cpu}%
║  🧠 <b>𝐌𝐄𝐌𝐎𝐑𝐘:</b> {memory}%
║  ⏱️ <b>𝐔𝐏𝐓𝐈𝐌𝐄:</b> {get_uptime()}
║
║  {'🟢 𝐄𝐗𝐂𝐄𝐋𝐋𝐄𝐍𝐓!' if latency < 100 else '🟡 𝐆𝐎𝐎𝐃' if latency < 500 else '🔴 𝐒𝐋𝐎𝐖'}
║
╚══════════════════════════════════════╝
"""
    try:
...
def running_command(message):
    """Show running bots"""
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐅𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝐛𝐨𝐭𝐬...", duration=1)
    running_bots = []
...
def lock_command(message):
    ...
    lock_text = f"""
╔══════════════════════════════╗
║         🔐 <b>𝐇𝐎𝐒𝐓𝐈𝐍𝐆 𝐁𝐎𝐓 𝐒𝐓𝐀𝐓𝐒</b> 🔐          ║
╠══════════════════════════════╣
║
║  𝐒𝐓𝐀𝐓𝐔𝐒: {status}
║  𝐁𝐘: {message.from_user.first_name}
║  𝐓𝐈𝐌𝐄: {datetime.now().strftime('%𝐇:%𝐌:%𝐒')}
║
╚══════════════════════════════════════╝
"""
...
def process_broadcast(message):
    ...
    if not broadcast_text:
        ...
        return
    progress_msg = bot.send_message(message.chat.id, "📢 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭...")
    success = 0
    failed = 0
...
"""
    ...
    try:
        bot.send_message(target_user, f"🎉 𝐘𝐨𝐮'𝐯𝐞 𝐛𝐞𝐞𝐧 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞𝐝 𝐟𝐨𝐫 {days} 𝐝𝐚𝐲𝐬 𝐛𝐲 LAZY PROVIDER !")
    except:
        pass
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 PROVIDERBOTZ 𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
def show_user_files(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 𝐟𝐢𝐥𝐞𝐬...", duration=1)
...
def stop_all_bots(call):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        ...
        return
    bot.answer_callback_query(call.id, "🛑 𝐒𝐭𝐨𝐩𝐩𝐢𝐧𝐠 𝐚𝐥𝐥 𝐛𝐨𝐭𝐬...")
    stopped = 0
...
def show_admin_logs(call):
    ...
    try:
        ...
        conn.close()
        if logs:
            text = "📋 <b>PROVIDERBOTZ : RECENT LOGS</b>\n"
            for log in logs:
                text += f"👤 {log[0]} | {log[1]}\n{log[2][:30]}...\n🕐 {log[3][:16]}\n"
        else:
            text = "📋 𝐍𝐨 𝐥𝐨𝐠𝐬."
...
def cleanup_on_exit():
    ...
    logger.info("𝐂𝐥𝐞𝐚𝐧𝐢𝐧𝐠 𝐮𝐩 PROVIDERBOTZ...")
    for script_key in list(bot_scripts.keys()):
        try:
            script_info = bot_scripts[script_key]
            kill_process_tree(script_info)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 PROVIDER HOSTING 🦁 𝐁𝐨𝐭...")
    ...
    while True:
        try:
            ...
            logger.error(f"PROVIDERBOTZ 𝐞𝐫𝐫𝐨𝐫: {e}", exc_info=True)
            time.sleep(5)
